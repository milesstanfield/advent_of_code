...
fn seed_nums(groups: &Vec<&str>) -> Vec<usize> {
    let nums = line_numbers(groups[0]);
    let mut vecs: DataVecs = vec![];

    for i in 0..nums.len() {
        if i % 2 == 0 {
            vecs.push((nums[i]..(nums[i] + nums[i + 1])).collect());
        }
    }
